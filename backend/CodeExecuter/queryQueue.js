...
const codeExecutorDir = `./codeExecutor`;
const {
  deleteFile,
  readFile,
  execCode,
...
queryQueue.process(WORKERS_NUMBER, async ({ data }) => {
  ...
  try {
    ...
    const code = await Code.createNewCode({
      code: readFile(query.filepath).toString(),
      language: query.language,
    });
    query.codeId = code._id;
    query.startTime = new Date();

    let response = await execCodeAgainstTestcases(
      query.filepath,
      query.testcase,
      query.language
    );
    ...
    if (!error.msg) {
      logger.error(
        "Error without msg in bull.process",
        error,
        dateTimeNowFormated()
      );
