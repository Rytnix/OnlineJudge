...
const loginValidatorHelper = async (
  req,
  res,
  next,
  credential,
  ...
  credentialName === "email" &&
    (existingUser = await User.findOneUser({ email: credential }));
  credentialName === "username" &&
    (existingUser = await User.findOneUser({ username: credential }));

  if (!existingUser)
    return res
      .status(401)
      .json({ error: `Wrong ${credentialName} or password.` });
  ...
  try {
    if (!password || !(email || username))
      return res.status(400).json({
        error: `Please enter all required fields. Missing : (email or username)${
          !password ? ", password" : ""
        }`,
      });

    await loginValidatorHelper(
      req,
      res,
      next,
...
const registerValidator = async (req, res, next) => {
  ...
  try {
    if (!name || !username || !email || !password || !passwordVerify)
      return res.status(400).json({
        error: `Please enter all required fields.  Missing :${
          !name ? " name" : ""
    ...
    if (username.length >= 10 || username.length < 4)
      return res.status(400).json({
        error:
          "Username should be less that 10 characters and greater than or equal to 4 characters",
      });
    ...
    if (username.toLowerCase().includes("aman")) {
      return res.status(400).json({
        error:
          "These Usernames (including 'aman') are reserved for Admin Only !",
      });
    }
